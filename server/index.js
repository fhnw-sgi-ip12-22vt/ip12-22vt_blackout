...
app.get('/iloveyou', async (req, res) => {
  let p = req.query.password;
  secureRoute(p);
...
app.get('/all-questions.json', async (req, res) => {
  let questions = await getQuestionsAsJson(true);
  res.json(JSON.parse(questions))
});

// GET questions.json
app.get('/questions.json', async (req, res) => {
  let questions = await getQuestionsAsJson(false);
  res.json(JSON.parse(questions))
});
...
app.post('/result', async (req, res) => {
  ...
  let doContinue = true;
  stats.forEach(stat => {
    if (stat.date == req.body.date) {
      doContinue = false;
    }
  })
  if (!doContinue) return res.send('OK')
  stats.push(req.body)
  ...
  let newQuestions = questions.map(question => {
    if (question.questionId == req.body.questionId) {
      question.totalPoints += req.body.points;
      question.maxPoints += 3;
    }
    return question;
  })
  let exponats = await database.get('exponats');
  let newExponats = await Promise.all(exponats.map(async exponat => {
    if (exponat.exponatId == req.body.exponatId) {
      exponat.totalPoints += req.body.points;
      exponat.maxPoints += 3;
    }
    let rooms = await database.get('rooms');
    let newRooms = rooms.map(room => {
      if (room.roomId == exponat.roomId) {
        room.totalPoints += req.body.points;
        room.maxPoints += 3;
      }
      return room;
    });
