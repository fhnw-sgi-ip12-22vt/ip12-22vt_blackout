...
public class AppAGUI extends Pane implements ViewMixin<AppModel, AppController> {
  ...
  public void setupModelToUiBindings(AppModel model) {
    onChangeOf(model.viewA).execute((oldValue, newValue) -> {
      if (controller.isIdleTimerRequiredA()) {
        controller.startIdleTimerA();
      }
      getChildren().clear();
