package com.blackout.util.mvcbase;
...
public final class ConcurrentTaskQueue<R> {
  ...
  // for non-thread-confined submissions, we might need an AtomicBoolean
  ...
  public ConcurrentTaskQueue(Duration maxToDoTime) {
    ...
    this.executor =
        Executors.newFixedThreadPool(1);  // use 2 for overlapping onDone with next to-do
